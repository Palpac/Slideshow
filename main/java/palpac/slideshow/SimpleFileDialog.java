...
public class SimpleFileDialog
{
    ...
    private String m_sdcardDirectory = "";                                                          // Perso
    //private String m_InternalAppFilePath = "";                                                    // Perso
    //private String m_Internal_Directory_Music = "";                                               // Perso
    private String m_persoDirectory = "";                                                           // Perso
    private String choosen_directory = "";                                                          // Perso
    ...
    public SimpleFileDialog(Context context, String file_select_type, SimpleFileDialogListener SimpleFileDialogListener)
    {
        ...
        m_sdcardDirectory = Environment.getExternalStorageDirectory().getAbsolutePath();            // Perso SD card path
        //m_InternalAppFilePath = context.getFilesDir().getAbsolutePath();                          // Perso Internal Storage App files (data/data/palpac.coherence_cardiaque/files)
        //m_Internal_Directory_Music = Environment.DIRECTORY_MUSIC;                                 // Perso Android Music Directory

        SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(m_context); //  Open preferences
        choosen_directory = preferences.getString("Directory_path", m_sdcardDirectory);

        m_persoDirectory = choosen_directory;                                                       // Perso
    }
    ...
    {
        if (m_dir.equals(""))	chooseFile_or_Dir(m_persoDirectory);                               // Initial directory is sdcard directory
    ...
    public void chooseFile_or_Dir(String dir)
    {
        ...
        class SimpleFileDialogOnClickListener implements DialogInterface.OnClickListener
        {
            public void onClick(DialogInterface dialog, int item)
            {
                ...
                LayoutParams params = ((AlertDialog) dialog).getListView().getLayoutParams();       // Perso resize list view to display buttons
                params.width = LayoutParams.MATCH_PARENT;                                           // Perso
                params.height = 400;                                                                // Perso
                ((AlertDialog) dialog).getListView().setLayoutParams(params);                       // Perso
        ...
        dialogBuilder.setCancelable(true);                                                          // Perso
        ...
        dirsDialog.show();
        //dirsDialog.getWindow().setLayout(600, 600); //Controlling width and height.               // Perso
    ...
    private List<String> getDirectories(String dir)
    {
        ...
        try
        {
            ...
            if (! m_dir.equals(m_sdcardDirectory) ) dirs.add("..");                                  // Perso
